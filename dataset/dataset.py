...
class Dataset:
    ...
    def load_data(
        ...
        try:
            ...
            logging.info(
                "Loaded %d rows with %d cells",
                self.raw_data.df.shape[0],
            ...
            self.attr_to_idx = {
                attr: idx for idx, attr in enumerate(self.raw_data.get_attributes())
            }
    ...
    def generate_aux_table_sql(self, aux_table, query, index_attrs=False):
        ...
        """
        try:
            self.aux_table[aux_table] = Table(
                aux_table.name, Source.SQL, table_query=query, db_engine=self.engine
            )
            if index_attrs:
                self.aux_table[aux_table].create_df_index(index_attrs)
                self.aux_table[aux_table].create_db_index(self.engine, index_attrs)
    ...
    def get_active_attributes(self):
        ...
        if self.env["infer_mode"] == "dk":
            if self._active_attributes is None:
                raise Exception(
                    "ERROR no active attributes loaded. Run error detection first."
                )
    ...
    def get_statistics(self):
        ...
        if not self.stats_ready:
            logging.debug(
                "computing frequency and co-occurrence statistics from raw data..."
            )
            tic = time.clock()
            self.collect_stats()
        ...
        for cond_attr in self.get_attributes():
            ...
            for trg_attr in self.get_attributes():
                if trg_attr != cond_attr:
                    self.pair_attr_stats[cond_attr][trg_attr] = self.get_stats_pair(
                        cond_attr, trg_attr
                    )
    ...
    def get_stats_single(self, attr):
        ...
        data_df = (
            self.get_quantized_data() if self.do_quantization else self.get_raw_data()
        )
        return (
            data_df[[attr]]
            .loc[data_df[attr] != NULL_REPR]
            .groupby([attr])
            .size()
            .to_dict()
        )
    ...
    def get_stats_pair(self, first_attr, second_attr):
        ...
        tmp_df = (
            data_df[[first_attr, second_attr]]
            .loc[
                (data_df[first_attr] != NULL_REPR) & (data_df[second_attr] != NULL_REPR)
            ]
    ...
    def get_domain_info(self):
        ...
        """
        query = (
            "SELECT count(_vid_), max(domain_size) FROM %s" % AuxTables.cell_domain.name
        )
    ...
    def get_inferred_values(self):
        ...
        query = (
            "SELECT t1._tid_, t1.attribute, domain[inferred_val_idx + 1] as rv_value "
            "FROM "
            "(SELECT _tid_, attribute, "
            "_vid_, init_value, string_to_array(regexp_replace(domain, '[{\"\"}]', '', 'gi'), '|||') as domain "
            "FROM %s) as t1, %s as t2 "
            "WHERE t1._vid_ = t2._vid_"
            % (AuxTables.cell_domain.name, AuxTables.inf_values_idx.name)
        )
        self.generate_aux_table_sql(
            AuxTables.inf_values_dom, query, index_attrs=["_tid_"]
        )
        self.aux_table[AuxTables.inf_values_dom].create_db_index(
    ...
    def get_embedding_model(self):
        ...
        if self._embedding_model is None:
            raise Exception(
                "cannot retrieve embedding model: it was never trained and loaded!"
            )
